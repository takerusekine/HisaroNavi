...
@section('content')
    ...
    @if ($title == '格安サロン')
        {!!Form::open(['class' => 'form-group', 'id' => 'area-choice-form', 'action' => 'FeatureController@getReasonableResult', 'method' => 'get'])!!}
    @else
        {!!Form::open(['class' => 'form-group', 'id' => 'area-choice-form', 'action' => 'FeatureController@getFeaturesResult', 'method' => 'get'])!!}
    @endif
                            ...
                            <label class="c-input c-checkbox">
                                ...
                                <?php $sum = 0; ?>
                                    <?php foreach ($sval as $nestkey => $nestval) :?>
                                        <?php
                                        if ($title == '格安サロン') {
                                            $featured = $salons->where('area_code', strval($nestval))
                                            ->filter(function ($item)
                                            {
                                                return $item['min_price'] <= 1500 && !empty($item['min_price']);
                                            });
                                            $sum += $featured->count();
                                            # code...
                                        } else {
